...
class Mamba(nn.Module):
    ...
    def forward(self, hidden_states, inference_params=None):
        ...
        if conv_state is not None:
            # If we just take x[:, :, -self.d_conv :], it will error if seqlen < self.d_conv
            # Instead F.pad will pad with zeros if seqlen < self.d_conv, and truncate otherwise.
            conv_state.copy_(F.pad(x_b, (self.d_conv - x_b.shape[-1], 0)))  # Update state (B D W)
        if causal_conv1d_fn is None:
            x_b = self.act(self.conv1d(x_b)[..., :seqlen])
        else:
            assert self.activation in ["silu", "swish"]
        ...
        # We want dt to have d as the slowest moving dimension
        # and L as the fastest moving dimension, since those are what the ssm_scan kernel expects.
        x_dbl_b = self.x_proj(rearrange(x_b, "b d l -> (b l) d"))  # (bl d)
        dt_b, B_b, C_b = torch.split(x_dbl_b, [self.dt_rank, self.d_state, self.d_state], dim=-1)
        dt_b = self.dt_proj.weight @ dt_b.t()
        ...
        out = F.linear(rearrange(y + y_b.flip([-1]), "b d l -> b l d"), self.out_proj.weight, self.out_proj.bias)
        
        # if ssm_state is not None:
        #     y, last_state = y
        #     y_b, last_state_b = y_b
        #     ssm_state.copy_(last_state + last_state_b)
